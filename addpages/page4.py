...
def app():
  ...
  def full_data_learning(genre_):
    ...
    data_folds = DatasetAutoFolds(ratings_file=f'db/p4/{genre_}_review_noh_1.csv', reader=reader)
  ...
  def mvCd_of_unshow(genre_,puid,punick):
    data = pd.read_csv(f'db/p4/{genre_}_review_noh_1.csv', names=['code','score','raw_user', 'userCd','user_id', 'user_nick', 'movie', 'genre','review'])
  ...
  def get_recomendationmovie(prediction):
    movies = pd.read_table('db/p4/movie_info_1.csv', sep=',', encoding='utf-8', engine='python')
    mvCd_list = []
    for i in prediction :
